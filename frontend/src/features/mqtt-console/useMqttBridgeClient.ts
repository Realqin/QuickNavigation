...
import { buildAppWebSocketUrl, buildMqttBridgeWebSocketUrl } from '../../utils/appWebSocket';

type ConnectionStatus = 'disconnected' | 'connecting' | 'connected' | 'error';

function buildConnectUrls(connectionId: number): string[] {
  const primary = buildMqttBridgeWebSocketUrl(connectionId);
  const urls = [primary];
  if (import.meta.env.DEV) {
    const viaProxy = buildAppWebSocketUrl(`/ws/mqtt/${connectionId}`);
    if (viaProxy !== primary) {
      urls.push(viaProxy);
    }
  }
...
export function useMqttBridgeClient() {
  ...
  const connect = useCallback(
    async (connectionId: number) => {
      ...
      const urls = buildConnectUrls(connectionId);
